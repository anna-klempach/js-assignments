...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let magnA = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2));
    let magnB = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2));

    return Math.acos(prod / (magnA * magnB));
}
...
function toNumber(value, def) {
    return (typeof +value === 'number' && value !== null && !Number.isNaN(+value)) ? +value : def ;
}
